...
const StatCard = ({ title, value, linkTo, bgColor = 'bg-blue-500', borderColor = 'border-blue-600', icon, subtext }) => (
  <Link to={linkTo} className={`block p-6 rounded-lg shadow-md bg-white border-t-4 ${borderColor} hover:shadow-lg transition-all duration-300`}>
    <div className="flex justify-between items-start">
      <div>
        <p className="text-sm font-medium text-gray-500">{title}</p>
        <h3 className="text-2xl sm:text-3xl font-bold text-gray-800 my-1">{value ?? '-'}</h3>
        {subtext && <p className="text-xs text-gray-500">{subtext}</p>}
      </div>
      {icon && <div className={`text-2xl ${bgColor} w-10 h-10 sm:w-12 sm:h-12 rounded-full flex items-center justify-center text-white shadow-md`}>{icon}</div>}
    </div>
  </Link>
);
...
const SimpleBarChart = ({ data, title }) => {
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-4 sm:p-6 col-span-1 md:col-span-2 lg:col-span-3">
    ...
    <div className="container mx-auto p-4 sm:p-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6">
        <div>
          ...
          {user && (
            <p className="mt-1 text-gray-600">Bem-vindo(a) de volta, <span className="font-medium text-gray-800">{user.nome}</span></p>
          )}
        </div>
        <button 
          onClick={fetchStats} 
          className="mt-4 sm:mt-0 px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 transition-colors flex items-center justify-center sm:justify-start text-sm w-full sm:w-auto"
        >
      ...
      {loading && (
        <div className="animate-pulse grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
          {[...Array(3)].map((_, index) => (
      ...
      {!loading && !stats && !error && (
        <div className="text-center py-8 sm:py-12 bg-white rounded-lg shadow">
          ...
          <h3 className="mt-2 text-sm font-medium text-gray-900">Sem dados disponíveis</h3>
          <p className="mt-1 text-sm text-gray-500">Não foi possível carregar as estatísticas.</p>
          <div className="mt-6">
            <button onClick={fetchStats} className="bg-blue-600 px-4 py-2 text-white rounded-md text-sm hover:bg-blue-700">Tentar Novamente</button>
          </div>
        </div>
      )}
    </div>
